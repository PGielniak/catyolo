...
class WhatsAppTestRequest(BaseModel):
    ...
    whatsapp_auth_scheme: str = "bearer"  # bearer | x-api-key | none
...
def test_telegram_connection(body: TelegramTestRequest) -> TelegramTestResponse:
    """Validate the bot token with getMe and the chat_id with sendMessage."""
    token = (body.telegram_bot_token or "").strip()
    chat_id = (body.telegram_chat_id or "").strip()
    if not token or not chat_id:
        return TelegramTestResponse(
            ok=False,
            message="telegram_bot_token and telegram_chat_id are required",
        )
    ...
    try:
        ...
        if not get_me.ok:
            return TelegramTestResponse(
                ok=False,
                message=f"getMe failed: {get_me.status_code} {get_me.text[:200]}",
            )

        try:
            bot_username = get_me.json().get("result", {}).get("username", "unknown-bot")
        except ValueError:
        ...
        send_msg = requests.post(
            ...
            data={
                "chat_id": chat_id,
                "text": "CatYolo Telegram connection test \u2705",
            },
            timeout=10,
        )
        if not send_msg.ok:
            return TelegramTestResponse(
                ok=False,
                message=(
                    f"Token valid (bot @{bot_username}) but sendMessage to chat "
                    f"{chat_id} failed: {send_msg.status_code} {send_msg.text[:200]}"
                ),
            )

        return TelegramTestResponse(
            ok=True,
            message=f"Bot @{bot_username} reached chat {chat_id}. Test message sent.",
        )
    except requests.RequestException as exc:
        return TelegramTestResponse(ok=False, message=f"Request error: {exc}")


@router.post("/test-whatsapp")
def test_whatsapp_connection(body: WhatsAppTestRequest) -> WhatsAppTestResponse:
    """Send a minimal test payload to the configured WhatsApp bridge URL."""
    url = (body.whatsapp_api_url or "").strip()
    if not url:
        return WhatsAppTestResponse(ok=False, message="whatsapp_api_url is required")

    auth_scheme = body.whatsapp_auth_scheme or "bearer"
    token = (body.whatsapp_api_token or "").strip()

    headers = {"Content-Type": "application/json", "User-Agent": "CatYolo/1.0"}
    if token and auth_scheme == "bearer":
        headers["Authorization"] = f"Bearer {token}"
    elif token and auth_scheme == "x-api-key":
        headers["X-API-Key"] = token

    payload = {
        "test": "CatYolo WhatsApp connection test",
        "message": "CatYolo WhatsApp connection test \u2705",
    }
    ...
    try:
        ...
        if 200 <= r.status_code < 300:
            return WhatsAppTestResponse(
                ok=True,
                message=f"Bridge responded {r.status_code} from {url.split('?')[0]}",
            )
        return WhatsAppTestResponse(
            ok=False,
            message=f"Bridge responded {r.status_code}: {r.text[:200]}",
        )
    except requests.RequestException as exc:
        return WhatsAppTestResponse(ok=False, message=f"Request error: {exc}")
